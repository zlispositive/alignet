...
Forked from big_vision/models/vit.py.
https://github.com/google-research/big_vision/blob/6d6c28a9634fd2f48f0f505f112d063dfc9bdf96/big_vision/models/vit.py
...
def load(init_params, init_file, model_cfg, dont_load=()):  # pylint: disable=invalid-name because we had to CamelCase above.
  ...
  if (
      model_cfg.get("scan")
      and "encoderblock" not in restored_params["Transformer"]
  ):
    restored_params = pyloop_to_scan(restored_params)
  if (
      not model_cfg.get("scan")
  ...
  # TODO(lbeyer): Take this from model_cfg to avoid need for init_params.
